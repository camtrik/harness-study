...
此模板由 complete-milestone 工作流用于在 `.planning/milestones/` 中创建归档文件。
...
- 阶段 2.1：关键安全补丁（在阶段 2 之后插入，用于紧急修复）
- 阶段 5.1：性能热修复（在阶段 5 之后插入，用于生产环境问题）
...
- PROJECT-STATE.md 分层（延迟到决策数超过 300 时）

**产生的技术债务：**
{{SHORTCUTS_NEEDING_FUTURE_WORK}}
[示例：]

- 某些工作流仍有硬编码路径（在阶段 5 中修复）

---

*关于当前项目状态，请参见 .planning/ROADMAP.md*
...
- 更新 ROADMAP.md，将已完成的里程碑折叠在 `<details>` 标签内
- 更新 PROJECT.md 为棕地格式，包含 Current State 段
- 在下一个里程碑中继续阶段编号（绝不要从 01 重新开始）
  </guidelines>
