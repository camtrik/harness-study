...
When `TDD_MODE` is `true`, the planner agent is instructed to apply `type: tdd` to eligible tasks using heuristics from `references/tdd.md`. The planner's `<required_reading>` is extended to include `@/Users/ebbi/Work/harness-study/gsd/.claude/get-shit-done/references/tdd.md` so gate enforcement rules are available during planning.
...
**If found:**
1. Read template: `/Users/ebbi/Work/harness-study/gsd/.claude/get-shit-done/templates/VALIDATION.md`
...
${TDD_MODE === 'true' ? `
<tdd_mode_active>
**TDD Mode is ENABLED.** Apply TDD heuristics from @/Users/ebbi/Work/harness-study/gsd/.claude/get-shit-done/references/tdd.md to all eligible tasks:
...
if [ "$POST_PLANNING_GAPS" = "true" ]; then
  node "/Users/ebbi/Work/harness-study/gsd/.claude/get-shit-done/bin/gsd-tools.cjs" gap-analysis --phase-dir "${PHASE_DIR}"
fi
```
