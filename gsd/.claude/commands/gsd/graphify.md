...
**CJS-only (graphify):** `graphify` subcommands are not registered on `gsd-sdk query`. Use `node /Users/ebbi/Work/harness-study/gsd/.claude/get-shit-done/bin/gsd-tools.cjs graphify …` as documented in this command and in `docs/CLI-TOOLS.md`. Other tooling may still use `gsd-sdk query` where a handler exists.
...
Project root: ${CWD}
gsd-tools path: /Users/ebbi/Work/harness-study/gsd/.claude/get-shit-done/bin/gsd-tools.cjs
