...
构建一个生成 ASCII fractals 的 CLI 工具。完整规范参见 `design.md`。
...
**操作:**
- 创建 `internal/cli/root.go` 和根命令
- 配置帮助文本,显示可用的子命令
- 将根命令连接到 `main.go`

**验证:**
- `./fractals --help` 显示使用说明,"sierpinski" 和 "mandelbrot" 列为可用命令
- `./fractals` (无参数)显示帮助
...
- 使用递归中点细分算法
- 创建 `internal/sierpinski/sierpinski_test.go` 并测试:
...
**操作:**
- 创建 `internal/cli/sierpinski.go` 和 `sierpinski` 子命令
...
- 将迭代计数映射到字符渐变 " .:-=+*#%@" (如果提供则为单个字符)
- 创建 `internal/mandelbrot/mandelbrot_test.go` 并测试:
  - 输出尺寸匹配请求的宽度/高度
...
**操作:**
- 创建 `internal/cli/mandelbrot.go` 和 `mandelbrot` 子命令
...
- 对于 Mandelbrot,`--char` 应该使用单个字符而不是渐变
- 为自定义字符输出添加测试
