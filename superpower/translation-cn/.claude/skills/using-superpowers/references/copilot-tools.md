...
| `WebFetch` | `web_fetch` |
| `Task` 工具（派发 subagent） | `task` 附带 `agent_type: "general-purpose"` 或 `"explore"` |
...
| `EnterPlanMode` / `ExitPlanMode` | 无等价工具——保持在主会话中 |

## 异步 shell 会话

Copilot CLI 支持持久化异步 shell 会话，这在 Claude Code 中没有直接等价工具：

| 工具 | 用途 |
|------|---------|
| `bash` 附带 `async: true` | 在后台启动长时间运行的命令 |
